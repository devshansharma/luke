...
const fileSuffix = "_environment.json"

func AddHandler(cfg Config) error {
	...
	fileName := fmt.Sprintf("%s/%s%s", dirPath, strings.ReplaceAll(cfg.Name, " ", "_"), fileSuffix)
...
func DeleteHandler(arg int) error {
	...
	resp, err := utils.AskYesOrNo(fmt.Sprintf("Are you sure, you want to delete %q environment", name))
	if err != nil {
		return err
	}
